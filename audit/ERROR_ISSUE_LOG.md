...
| E9 | **Optimistic swipe removes the candidate before server confirm and swallows failures** — a failed "like" silently never matches; double-tap can double-submit. No in-flight guard, no resync. | `src/SuggestionScreen.jsx:510-560` | Code-Quality |
...
| E12 | **Two socket.io connections per user; churn on thread switch** — app-level + per-conversation sockets; the conversation socket re-`io()`s on every `conversationId`/`currentUserId` change, opening/closing in a loop on rapid switching. | `src/App.jsx:869-891`; `messaging/ConversationScreen.jsx:867-909` | Code-Quality |
| E13 | **`markConversationRead` fire-and-forget races the list refresh** — unread can flicker back if the PUT hasn't committed before the re-fetch; two writers to the badge. | `src/messaging/MessagingApp.jsx:95-99` | Code-Quality |
| E14 | **`handleBlockReportSubmit` null-derefs `currentConvo.otherUser.userId`** — async handler can fire after state nulls `currentConvo` → unhandled rejection, block/report silently lost. (`handleUnmatchConfirm` uses optional chaining — inconsistent.) | `src/messaging/MessagingApp.jsx:134-147` | Code-Quality |
| E15 | **Inactivity effect's blanket `eslint-disable` hides dep-array risk** — fine today, but a future state reference would capture a stale value with no lint to catch it (no ESLint installed). | `src/App.jsx:712-759` | Code-Quality |
...
| E23 | Auth subview `document.title` stale on Forgot-password / Check-email / in-card login↔register toggle. | `src/App.jsx:573-587`, `AuthScreen.jsx:105-109,409-427` |
...
| E1▲ | **Migration non-idempotency now concretely proven.** `005_profile_photos.sql` adds `public_url` as the *tail* statement after a duplicate-column ALTER; a re-migrated DB silently skips it → `/photos/upload-intent` 500s on `no such column: public_url`. (Upgrades R1 E1 from latent to demonstrated.) | `db.js:57-70`; `server/src/migrations/005` | Backend + Code |
...
| E36 | **`message_attachments.mime_type` CHECK omits `image/gif`** though `ALLOWED_MIME` accepts it → GIF INSERT throws → opaque 500. | `server/src/migrations/004:19` vs `photos.js:9` | Backend |
