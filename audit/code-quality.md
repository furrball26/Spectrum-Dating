...
- where: `Spectrum-Dating-Server/src/db.js:57-70` (the `runMigrations` catch/`continue`); triggered by multi-`ALTER` files e.g. `migrations/014_dealbreakers.sql`, `018_richer_profile.sql`, `023_search_radius.sql`, `026_age_pref.sql`.
...
- where: `Spectrum-Dating/src/messaging/ConversationScreen.jsx:1034-1099` (esp. 1059-1076); gated off by `ATTACHMENTS_ENABLED = false` at line 37.
...
- where: `Spectrum-Dating/src/messaging/MessagingApp.jsx:36` (`arr.filter(c => c.hasUnread)`) vs `Spectrum-Dating/src/api.js:199-209` (`normaliseConversationList`).
...
- where: `Spectrum-Dating/src/SafetyScreen.jsx:169-173` (`copyText`), consumed at `:359-371` (`handleCopyScript`) and `:386-407` (`handleSharePlan`).
...
- where: `Spectrum-Dating/src/App.jsx:869-891` (app-level socket) and `Spectrum-Dating/src/messaging/ConversationScreen.jsx:867-909` (per-conversation socket).
...
- where: `Spectrum-Dating/src/messaging/MessagingApp.jsx:95-99` (`handleSelectConversation`) and `:45-50` (refresh-on-list effect).
...
- where: `Spectrum-Dating/src/SuggestionScreen.jsx:510-560` (`handleInterested`/`handleNotNow`/`handleSkip`).
...
- where: `Spectrum-Dating/src/messaging/MessagingApp.jsx:134-147`.
...
- where: `Spectrum-Dating/src/App.jsx:712-759` (note `// eslint-disable-line react-hooks/exhaustive-deps` at 759).
...
- where: `Spectrum-Dating-Server/src/routes/matching.js:29-30` (`parseInt(req.query.offset)`, `parseInt(req.query.limit)`).
...
- where: `Spectrum-Dating-Server/src/routes/matching.js:60, 218, 290, 321` and described in candidates; the regex `replace(/[\s,]*\d{4,}(-\d+)?\s*$/, '').replace(/[\s,]+$/, '').trim()`.
...
- where: `Spectrum-Dating/src/messaging/ConversationScreen.jsx:1566`.
...
- where: `Spectrum-Dating-Server/src/index.js:94-100`.
...
- `Spectrum-Dating/src/messaging/ConversationScreen.jsx:1059` — `// TODO: backend message-attachment linking — sendMessage does not yet accept an attachmentId, so the photo is shown optimistically via publicUrl and the text body (if any) is persisted on its own.` (corresponds to the 🔴 attachment finding above).
