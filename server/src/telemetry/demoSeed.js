...
//     `telemetry-demo-` prefixed members).
...
// Deletes ONLY is_demo=1 telemetry + `telemetry-demo-`-prefixed members. Called
// both standalone (wipeDemoData, wrapped in a transaction) and as the idempotent
// pre-clean inside loadDemoData's transaction. Never touches real (is_demo=0)
// rows or the existing @sample seed personas.
function deleteDemoRows(db) {
  ...
  return { pageViews, visitDaily, incidents, members };
}

// ── wipeDemoData(db) ─────────────────────────────────────────────────────────
// Deletes ONLY is_demo=1 telemetry + telemetry-demo- members, in one transaction.
// Returns { pageViews, visitDaily, incidents, members } deleted-row counts.
...
// then inserts a fresh 30-day demo dataset — page_views + matching visit_daily
// rollups + two uptime incidents + the demo members — all inside ONE transaction.
// Returns inserted counts { pageViews, visitDaily, incidents, members, uniqueSessions }.
